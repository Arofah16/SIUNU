...
<?php
	$idkat = $barang->id_kategori;
	$idrak = $barang->id_ruang;

	$kat = $this->M_Admin->get_tableid_edit('tbl_kategori','id_kategori',$idkat);
	$rak = $this->M_Admin->get_tableid_edit('tbl_rak','id_ruang',$idrak);
?>
	        ...
	        <div class="box box-primary">
			    ...
			    <div class="box-body">
					<table class="table table-striped table-bordered">
						...
						<tr>
							<td>Sampul Buku</td>
							<td><?php if(!empty($barang->sampul !== "0")){?>
									<a href="<?= base_url('assets_style/image/buku/'.$barang->sampul);?>" target="_blank">
										<img src="<?= base_url('assets_style/image/buku/'.$barang->sampul);?>" style="width:170px;height:170px;" class="img-responsive">
									</a>
						...
						<tr>
							<td>Lampiran</td>
							<td><?php if(!empty($barang->lampiran !== "0")){?>
									<a href="<?= base_url('assets_style/image/buku/'.$barang->lampiran);?>" class="btn btn-primary btn-md" target="_blank">
...
<table id="example1" class="table table-bordered table-striped">
	...
	<?php 
	$no = 1;
	$barangid = $barang->buku_id;
	$pin = $this->db->query("SELECT * FROM tbl_pinjam WHERE buku_id ='$barangid' AND status = 'Dipinjam'")->result_array();
	foreach($pin as $si)
	{
